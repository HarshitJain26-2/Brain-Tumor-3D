...
export default function MedicalReport({ isOpen, onClose, stats, jobId, snapshot }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 sm:p-8 no-print">
      <motion.div 
      ...
      >
        ...
        {/* Report Content */}
        <div id="printable-report" className="flex-1 overflow-y-auto p-12 bg-white print:p-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div className="space-y-8">
               ...
               <section>
                  ...
                  <div className="bg-slate-900 border border-slate-800 p-6 rounded-[1.5rem] text-white space-y-4 shadow-xl">
                     <div>
                        <p className="text-[9px] text-white/40 font-bold uppercase mb-1 tracking-widest">Primary Focal Point</p>
                        <p className="text-xl font-black text-white leading-tight uppercase">{getQuadrantText()}</p>
                     </div>
                     <div className="flex justify-between items-end border-t border-white/5 pt-4 mt-4">
                        <div>
                           <p className="text-[9px] text-white/40 font-bold uppercase mb-1">Coordinated HWD</p>
                           <p className="text-xs font-bold text-accent-blue">{stats.location?.coordinates || 'N/A'}</p>
                        </div>
                        <div className="text-right">
                           <p className="text-[9px] text-white/40 font-bold uppercase mb-1">Overall Diameter</p>
                           <p className="text-xs font-bold text-white">{stats.location?.bbox_mm || 'N/A'}</p>
                        </div>
                     </div>
                  </div>
               </section>
            ...
            <div className="space-y-8">
               <section>
                  ...
                  <div className="aspect-square bg-slate-900 rounded-[2rem] overflow-hidden border-4 border-white shadow-apple relative flex items-center justify-center">
                    {snapshot ? (
                      ...
                      <div className="text-center p-8">
                        ...
                        <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest">Capturing Viewport...</p>
                      </div>
                    )}
                    <div className="absolute bottom-4 right-4 text-[9px] bg-black/60 backdrop-blur-md text-white px-3 py-1 rounded-full border border-white/10 font-bold uppercase tracking-widest">
      ...
      <style>{`
        @media print {
          ...
          * {
          ...
          /* Ensure text scale is readable on paper */
          h1, h2, h3, p, span {
